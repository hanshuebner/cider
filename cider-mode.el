...
(defun cider--read-locals-from-next-sexp ()
  "Return a list of all symbols inside the next logical sexp."
  (save-excursion
    (ignore-errors
      ...
      (let ((out nil)
            (end (point)))
        (forward-sexp -1)
        ;; FIXME: This returns symbols found inside the :or clause of a
        ;; destructuring map.
...
(defun cider--read-locals-from-bindings-vector ()
  "Return a list of all symbols inside the next bindings vector."
  (save-excursion
    (ignore-errors
...
(defun cider--read-locals-from-arglist ()
  "Return a list of all symbols in current form's arglist(s)."
  (let ((out nil))
    (save-excursion
      (ignore-errors
        (cider-start-of-next-sexp)
